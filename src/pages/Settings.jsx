      ...
      <main className="max-w-4xl mx-auto px-6 pt-24 text-slate-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-6">
              ...
              <form onSubmit={handleUpdate} className="space-y-6">
                {isStudent && (
                  <div className="flex items-center justify-between py-4 border-b border-slate-100 dark:border-slate-800">
                    <div>
                      <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Index Number</p>
                      <p className="text-lg font-mono text-blue-600 dark:text-blue-400 font-bold">{user?.indexNumber || "Pending..."}</p>
                    </div>
                ...
                <div className="py-4 border-b border-slate-100 dark:border-slate-800">
                  <p className="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1">Full Name</p>
                  {isEditing ? (
                    <input
                      type="text"
                      className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white focus:border-blue-500 outline-none transition-all"
                      value={formData.name}
                ...
                <div className="py-4 border-b border-slate-100 dark:border-slate-800">
                  <p className="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1">Email Address</p>
                  {isEditing ? (
                    <input
                      type="email"
                      className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white focus:border-blue-500 outline-none transition-all"
                ...
                {isStudent && (
                  <div className="flex items-center justify-between py-4 border-b border-slate-100 dark:border-slate-800">
                    <div>
                      <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Assigned Batch</p>
                      <p className="text-slate-700 dark:text-slate-300 font-medium">{user?.batch}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between py-4">
                  <div>
                    <p className="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1">Registration Date</p>
                ...
                {isEditing && (
                  <div className="pt-6 flex gap-4">
                    <button
                      ...
                      className="flex-1 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-300 dark:disabled:bg-slate-700 disabled:cursor-not-allowed text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-blue-500/20"
                    >
                    ...
                    <button
                      ...
                      className="px-6 bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-white font-bold py-3 rounded-xl transition-all"
                    >
                      Cancel
                    </button>
                  </div>
            ...
            {/* Security Notice */}
            <div className="bg-amber-50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/10 rounded-2xl p-6 flex gap-4 items-start">
              <span className="text-xl">🛡️</span>
              <div>
                <h4 className="text-sm font-bold text-amber-600 dark:text-amber-500 mb-1">Verification Status</h4>
